...
ggplot(alpha_div_ASV)+
  geom_histogram(aes(x=log(Chao1+1)))+theme_classic()+ylab("Count")+xlab("Chao1 - log transformed")

ggplot(alpha_div_ASV)+
  geom_histogram(aes(x=Shannon))+theme_classic()+ylab("Count")
...
ggplot(data=alpha_sub_Chao1)+
  geom_boxplot(aes(x=Author, y=Chao1_transformed, color=Disease))+theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+xlab("")+ylab("Chao1 (log transformed)")

ggplot(data=alpha_sub_Shannon)+
  geom_boxplot(aes(x=Author, y=Shannon, color=Disease))+theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+xlab("")

# Not divided by projects:
alpha_sub_Chao1$Disease <- factor(alpha_sub_Chao1$Disease, levels = c("HC", "UC", "CD"))
alpha_sub_Shannon$Disease <- factor(alpha_sub_Shannon$Disease, levels = c("HC", "UC", "CD"))
...
p1 <- ggplot(data=alpha_sub_Chao1 %>% filter(!is.na(Disease)))+
  geom_boxplot(aes(x=Disease, y=Chao1_transformed, color=Disease))+theme_classic()+ theme(legend.position="none")+xlab("") + ylab("Chao1 richness (log transformed)")+add_pvalue(p_val_ref)#+ scale_color_grey(start = 0.1, end = .8, na.value="black")
...
p2 <- ggplot(data=alpha_sub_Shannon %>% filter(!is.na(Disease)))+
  geom_boxplot(aes(x=Disease, y=Shannon, color=Disease))+theme_classic()+ theme(legend.position="none")+xlab("") + ylab("Shannon diversity")+add_pvalue(p_val_ref2)#+ scale_color_grey(start = 0.1, end = .8, na.value="black")
...
data.scores$Disease <- factor(data.scores$Disease, levels = c("HC", "CD", "UC"))

p1 <- ggplot(data=data.scores, aes(x=NMDS1, y=NMDS2, color=Disease))+
  geom_point(size=0.7)+theme_classic()+stat_ellipse(linetype="dashed", size=2)+ggtitle("Bray Curtis dissimilarity")
p1
...
data.scores_jac$Disease <- factor(data.scores_jac$Disease, levels = c("HC", "CD", "UC"))

p2 <- ggplot(data=data.scores_jac, aes(x=NMDS1, y=NMDS2, color=Disease))+
  geom_point(size=0.7)+theme_classic()+stat_ellipse(linetype="dashed", size=2)+ggtitle("Jaccard index")
