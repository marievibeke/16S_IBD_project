...
Make NMDS showing the "chosen ones"
```{r}
library(vegan)
ps_relab <- subset_samples(ps_relab_reduced, !is.na(Disease))
ps_relab <- subset_samples(ps_relab, !is.na(geo_loc_name_country))
ps_relab <- subset_samples(ps_relab, !is.na(host_sex))
ps_relab <- subset_samples(ps_relab, !is.na(Host_Age))

#CD:
#Bray Curtis: 
test <- res_CD %>% filter(part =="Crohn's disease, NB")

ps_CD <- subset_samples(ps_relab, Disease !="UC")
ps_CD <- subset_samples(ps_CD, Run != "ERR162916")
my_subset <- as.data.frame(as.matrix(otu_table(ps_CD)))
my_subset <- my_subset[, test$bug]

ps_CD <- merge_phyloseq(otu_table(my_subset, taxa_are_rows =F), tax_table(ps_CD), sample_data(ps_CD))

set.seed(0)
BRAY_NMDS_relab=vegan::metaMDS(otu_table(ps_CD), k=2,trymax=30, distance="bray")
BRAY_NMDS_relab$stress #0.2168137

data_scores <- as.data.frame(scores(BRAY_NMDS_relab))
data_scores$Disease <- sample_data(ps_CD)$Disease
data_scores$Disease <- factor(data_scores$Disease, levels = c("HC", "CD"))
data_scores$Run <- row.names(data_scores)
species_scores <- as.data.frame(scores(BRAY_NMDS_relab, "species"))
species_scores$species <- rownames(species_scores)
species_scores$species <- gsub(".*_","",species_scores$species)

ggplot(data_scores) +
  geom_text(data = species_scores, aes(x = NMDS1, y = NMDS2, label = species), alpha = 0.5, size = 3) +
  geom_point( aes(x = NMDS1, y = NMDS2, color = Disease), size = 0.7) +
  theme_classic()#+stat_ellipse(aes(x = NMDS1, y = NMDS2, color = Disease), linetype="dashed", size=2)

#Jaccard
set.seed(0)
BRAY_NMDS_relab=vegan::metaMDS(otu_table(ps_CD), k=2,trymax=30, distance="jaccard", binary=T)
BRAY_NMDS_relab$stress #0.1822771
...
#UC:
#Bray Curtis: 
test <- res_UC %>% filter(part =="Ulcerative colitis, NB")

ps_UC <- subset_samples(ps_relab, Disease !="CD")
my_subset <- as.data.frame(as.matrix(otu_table(ps_UC)))
my_subset <- my_subset[, test$bug]

ps_UC <- merge_phyloseq(otu_table(my_subset, taxa_are_rows =F), tax_table(ps_UC), sample_data(ps_UC))

set.seed(0)
BRAY_NMDS_relab=vegan::metaMDS(otu_table(ps_UC), k=2,trymax=30, distance="bray")
BRAY_NMDS_relab$stress #0.2029482

data_scores <- as.data.frame(scores(BRAY_NMDS_relab))
data_scores$Disease <- sample_data(ps_UC)$Disease
data_scores$Disease <- factor(data_scores$Disease, levels = c("HC", "UC"))
data_scores$Run <- row.names(data_scores)
species_scores <- as.data.frame(scores(BRAY_NMDS_relab, "species"))
species_scores$species <- rownames(species_scores)
species_scores$species <- gsub("Prevotella_","Prevotella.",species_scores$species)
species_scores$species <- gsub(".*_","",species_scores$species)
...
#Jaccard
set.seed(0)
BRAY_NMDS_relab=vegan::metaMDS(otu_table(ps_UC), k=2,trymax=30, distance="jaccard", binary=T)
BRAY_NMDS_relab$stress #0.1928975
